...
Manual, free-tool, automated, and AI-assisted approaches for running YouTube / Video Engine, ordered from
...
1. n8n watches the editorial calendar (Notion/Airtable) for posts tagged "M13 — ready".
...
Video is the one format in Track M that AI drafting only partially covers — AI can draft the script, title, description, and the 10 SEO tags, but filming/editing is a separate manual or freelance step not covered by this methods.md. Screen-recording tutorials are the cheapest format to start with since they need no on-camera talent.
