...
\- Server pe Docker + Portainer install karo

\- n8n, Odoo, PostgreSQL, Mautic containers deploy karo

\- n8n me saare credentials add karo — Gmail, Sheets, WhatsApp API, Notion, CRM API

\- Stage 1-2 template import karo (lead capture) → real test lead se run karo → check karo data sahi jagah aa raha hai

\- Confirm hone ke baad Stage 3, 4... aage badhao, ek-ek karke

\- Stage 7 pe DocuSeal alag se self-host karna hoga (e-signature)

\- Stage 11 pe review-request ka chhota 2-node flow khud banana hoga

\- Sales funnel solo chalne ke baad marketing se connect karo

\- Sabse last me Metabase/Grafana dashboard lagao



\*\*Marketing Funnel:\*\*

\- Notion/AnythingLLM me Knowledge Base banao — brand voice, offers, FAQs daalo (real content, placeholder nahi)

\- Ek AI Brain choose karo (Claude/ChatGPT ya Ollama) — chaaron ek saath mat lagao

\- Content, SEO, Strategy — teeno alag n8n workflow banao, ek-ek test karo

\- Website/Blog publishing pehle set karo (WordPress + webhook)

\- Fir Social Scheduling (Mixpost/Postiz) add karo

\- Fir Email Marketing (Mautic) add karo

\- Fir CRM Update hook add karo

\- Har channel ka real account connect karo — ek verify karke hi agla lagao

\- Analytics (Matomo/GA) sabse last me — jab content publish ho chuka ho



\*\*Overlap:\*\* Sales funnel pehle banao to marketing ke liye CRM + n8n already ready milega.
...
The original sales-funnel doc flagged four gaps in §9 of that file. Rather than re-stating those, I re-cloned the live repository (`nivyindia/all\_n8n\_templates\_collection`, sparse-checked out to verify actual current contents — it has grown since that doc was written, into 2,750+ files across \~40 category folders, not the \~280 the original doc described) and searched the web for anything outside it.
...
All links below resolve inside [**nivyindia/all\_n8n\_templates\_collection**](https://github.com/nivyindia/all_n8n_templates_collection) unless marked "external." Quick-start: sign up for [n8n](https://n8n.io/) → download the `.json` → **Workflows → Import from File** → configure credentials → activate.
...
|Appointment/booking pre-qualification|[Qualifying Appointment Requests with AI \& n8n Forms](https://github.com/nivyindia/all_n8n_templates_collection/blob/main/Forms_and_Surveys/Qualifying%20Appointment%20Requests%20with%20AI%20&%20n8n%20Forms.json)|
...
|WhatsApp|Higher-quality reply pass|[Respond to WhatsApp Messages with AI Like a Pro!](https://github.com/nivyindia/all_n8n_templates_collection/blob/main/WhatsApp/Respond%20to%20WhatsApp%20Messages%20with%20AI%20Like%20a%20Pro%21.json)|
|WhatsApp + IG DM + Messenger|One unified inbox|[Receive and Send Messages Across WhatsApp, Instagram and Facebook Messenger with Fiwano](https://github.com/nivyindia/all_n8n_templates_collection/blob/main/WhatsApp/Receive%20and%20Send%20Messages%20Across%20WhatsApp,%20Instagram%20and%20Facebook%20Messenger%20with%20Fiwano.json)|
...
|Long-term memory agent (community FAQ / knowledge bot)|Telegram|[AI Agent Chatbot + Long Term Memory + Note Storage + Telegram](https://github.com/nivyindia/all_n8n_templates_collection/blob/main/Telegram/🤖🧠%20AI%20Agent%20Chatbot%20+%20LONG%20TERM%20Memory%20+%20Note%20Storage%20+%20Telegram.json)|
...
* Every sales-funnel link in Part III.6 was checked against a fresh sparse `git clone` of `nivyindia/all\_n8n\_templates\_collection` on 2026-07-24, not re-copied from the source `.md` files — three previously-flagged gaps turned out to be resolvable because the repo has grown since the original doc was written (the repo's own numbers have moved from \~280 templates in \~18 folders to 2,750+ files across \~40 folders, including new `Finance\_Accounting` and expanded `Other\_Integrations\_and\_Use\_Cases` folders that didn't exist yet when the source doc was written).
...
* [nivyindia/all\_n8n\_templates\_collection](https://github.com/nivyindia/all_n8n_templates_collection) — the live template repository every link in Part III (and the Telegram/Discord links in Part V) points into
