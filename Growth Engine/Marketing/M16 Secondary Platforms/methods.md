...
Manual, free-tool, automated, and AI-assisted approaches for running Secondary Platforms (Pinterest, Threads, WhatsApp Broadcast), ordered from
...
1. n8n watches the editorial calendar (Notion/Airtable) for posts tagged "M16 — ready".
...
Pinterest and Threads have no internal playbook to draft methods against yet (see the source coverage flag in README.md). Until goals/cadence are defined, treat any posting on these two platforms as manual and experimental rather than running it through the automated pipeline below. The WhatsApp Channel methods, by contrast, follow the same manual → free-tool → automated → AI-assisted progression as the other five stages.
